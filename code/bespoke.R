...
df_id <- df_input %>% select(contains("id"))
...
df_char1_vec <- df_input %>% select(matches(df_char1_name)) %>% pull()
df_char2_name <- df_characters[2]
df_char2_vec <- df_input %>% select(matches(df_char2_name)) %>% pull()
df_id_name <- sample(colnames(df_id), 1)
df_id_vec <- df_input %>% select(matches(df_id_name))
