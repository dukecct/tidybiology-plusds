...
source(here::here("code", "bespoke.R")) #loads custom objectssource(here::here("code", "bespoke.R")) #loads custom objects
```
...
```{r wakeup_quiz3, echo=TRUE, eval=FALSE}
morning_routine <- wakeup(self) %>%
  put_on("clothes") %>%
  eat("breakfast") %>%
  fmk() %>% 
...
`summarize()` produces a new dataframe that aggregates that values of a column based on a certain condition.
...
`group_by()` and `summarize()` can be used in combination to summarize by groups
